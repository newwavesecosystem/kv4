...
import {guestRoomURL} from "~/server/ServerInfo";
...
function JoinId() {
    ...
    const [guestRoomMessage, setGuestRoomMessage] = useState('Please wait for a moderator to approve you joining the meeting.');
    const [guestRoomToken, setGuestRoomToken] = useState('');

    let sinterval: any = null;
    let sginterval: any = null;
    ...
    useEffect(() => {
        if (guestRoom) {
            ...
            return () => clearInterval(sginterval);
        }
    }, [guestRoom]);
            ...
            .then(function (response) {
                ...
                if (responseData?.success) {
                    setMeetingData({
                        ...
                        display_image: responseData?.data?.display_image,
                        owner: responseData?.owner,
                        passCode: responseData?.access_code
                    })
                } else {
                    // toast({
                    //   variant: "destructive",
            ...
            .then(function (response) {
                ...
                if (responseData?.success) {
                    ...
                    if (responseData?.wait) {
                        setGuestRoomToken(responseData.data);
                        setGuestRoom(true);
                    } else {
    ...
    const guestRoomAPI = () => {
        setLoading(true);
        axios.get(`${ServerInfo.guestRoomURL}?sessionToken=${guestRoomToken}`)
            .then(function (response) {

                if (!response?.request?.responseURL.includes("guestWait")) {
                    console.log("redirection dectected:",response?.request?.responseURL);
                ...
                if (responseData?.response?.returncode === "SUCCESS") {
                    if(responseData?.response?.guestStatus == "ALLOW"){
                        window.location.replace(responseData?.response?.url);
                    }

                    if(responseData?.response?.guestStatus == "DENY"){
                        setGuestRoom(false);
                        toast({
                            variant: "destructive",
                            title: "Uh oh! Something went wrong.",
                            description: `You have been denied of joining this meeting`,
                        });
                    }

                    if(responseData?.response?.guestStatus == "WAIT"){
                        setGuestRoomMessage(responseData?.response?.lobbyMessage ==""?responseData?.response?.message:responseData?.response?.lobbyMessage);
                    }
                }
                    ...
                    <div className="m-auto flex flex-col items-center justify-center gap-3 px-5 text-center text-black">
                      <span className=" text-xl font-semibold md:text-2xl">
                        ...
                        <div className=" flex w-full flex-col items-center justify-center gap-5 animate-flip-down">
                            ...
                            ) : data.fullName == "" && meetingData.display_image != null ? (
                                <div className="flex aspect-auto animate-pulse h-64 w-full flex-col items-center justify-center rounded-2xl bg-[#B9C9C2] md:w-[500px]">
                                    <Image
                            ...
                            {/*Show input boxes for name, email & accesscode*/}
                            {!waitingRoom && !guestRoom && (<div className="flex w-full gap-4 text-start">
                            ...
                            {!waitingRoom && !guestRoom && (<div className="flex w-full gap-4 text-start text-sm">
                                {/*<div*/}
                                {/*    className={cn(*/}
                                {/*        "flex flex-col gap-1",*/}
                            ...
                            {!waitingRoom && !guestRoom && (<div className="flex w-full">
                                {loading ? <SpinnerIcon coloured={true} className="animate-spin text-center ml-32"/> :
                                    <button
                                        onClick={() => {
                                            if (data.fullName == "") {
                                                toast({
                            ...
                            {/*Ended showing input boxes for name, email & accesscode and Buttom*/}

                            {/*Start waiting room display*/}
                            {waitingRoom && (
                                <div className="flex w-full gap-4 mt-5 text-center content-center items-center">
                                    <SpinnerIcon coloured={true} className="animate-spin text-center ml-4"/>
                                    <span className="">Waiting for the Host to start the meeting</span>
                                </div>)
                            }

                            {/*Show Cancel waiting room button*/}
                            ...
                            {/*Start guest room display*/}
                            {guestRoom && (
                                <div className="flex w-full gap-4 mt-5 text-center content-center items-center">
                                    <SpinnerIcon coloured={true} className="animate-spin text-center ml-4"/>
                                    <span className="">{guestRoomMessage}</span>
                                </div>)
                            }
